...
func NewCaptchaService(rdb *redis.Client, cfg *config.CaptchaConfigSpec, logger *zap.Logger) *CaptchaService {
	...
	if service.imagePool != nil {
		imagePool = &captchaImagePoolAdapter{pool: service.imagePool}
	}
...
type captchaImagePoolAdapter struct {
	pool *RedisImagePool
}

var _ appports.BackgroundImagePool = (*captchaImagePoolAdapter)(nil)

func (a *captchaImagePoolAdapter) Random(ctx context.Context) ([]byte, error) {
	if a == nil || a.pool == nil {
...
func (a *captchaImagePoolAdapter) Snapshot(ctx context.Context) (domain.ImagePoolSnapshot, error) {
	if a == nil || a.pool == nil {
...
func (a *captchaImagePoolAdapter) Refresh(ctx context.Context) error {
	if a == nil || a.pool == nil {
...
func (a *captchaImagePoolAdapter) RefreshWithProvider(ctx context.Context, provider appports.ImageProvider) error {
	if a == nil || a.pool == nil {
		return fmt.Errorf("captcha image pool is not configured")
	}
...
func (a *captchaImagePoolAdapter) Start(ctx context.Context, interval time.Duration, refreshOnStartup bool) {
	if a != nil && a.pool != nil {
		a.pool.StartRefresh(ctx, interval, refreshOnStartup)
	}
}

func (a *captchaImagePoolAdapter) Stop() {
	if a != nil && a.pool != nil {
		a.pool.StopRefresh()
	}
}
